...
public:
    EchoServer(EventLoop *loop,
               ...
               int threadNum = 1) : _server(loop, inetAddress, nameArg),
                                    _loop(loop)
    {
...
int main(int argc, char* argv[]) {
    ...
    EchoServer server(&loop, addr, "EchoServer", threadNum);
    LOG_INFO("EchoServer starting with %d worker thread(s)", threadNum);
    server.start();
    loop.loop();
    return 0;
}
